#!/usr/bin/env python3
"""
migrate_investment_tracking_products.py
========================================
Creates investment_tracking_products and loads data from 跟踪产品.xlsx.

Safe to re-run: CREATE TABLE uses IF NOT EXISTS; rows upsert on beian_hao.

Usage:
    python scripts/db/migrate_investment_tracking_products.py
"""
...
DDL = """
CREATE TABLE IF NOT EXISTS investment_tracking_products (
    ...
    source_file     TEXT NOT NULL DEFAULT '跟踪产品.xlsx',
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT investment_tracking_products_beian_hao_uq UNIQUE (beian_hao)
);

CREATE INDEX IF NOT EXISTS idx_investment_tracking_products_manager
    ON investment_tracking_products (manager_name);

CREATE INDEX IF NOT EXISTS idx_investment_tracking_products_nav_date
    ON investment_tracking_products (nav_date DESC);
"""

UPSERT_SQL = """
INSERT INTO investment_tracking_products (
...
def main() -> None:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(DDL)
            print("  + investment_tracking_products table ready")

            execute_values(cur, UPSERT_SQL, rows, page_size=500)
            cur.execute("SELECT COUNT(*) FROM investment_tracking_products")
