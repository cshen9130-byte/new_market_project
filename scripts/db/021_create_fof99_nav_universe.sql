...
-- weekly_plus  = email usually already has the week; pay Friday only if list tip is behind
...
CREATE TABLE IF NOT EXISTS fof99_nav_universe (
  ...
  product_name TEXT,
  policy       TEXT        NOT NULL CHECK (policy IN ('weekly', 'weekly_plus', 'skip', 'update_slow')),
  reason       TEXT,
