#!/usr/bin/env python3
"""Export the original 1-3m job funds that landed in 1个月内 (weekly). No paid calls.

Same columns as fof99_3_6m_have_data.csv. Universe is the frozen 350
(`fof99_nav_universe.reason` = 1-3m job now within 1 month), not today's list filter.
"""
...
OUT = ROOT / "scripts" / "ma" / "fof99_1m_have_data.csv"
REASON_1M = "1-3m job now within 1 month; weekly Friday FundMultiPrice"
...
def main() -> int:
    ...
    cur.execute(
        """
        SELECT UPPER(BTRIM(u.reg_code)), COALESCE(i.product_name, u.product_name, ''),
               i.latest_nav_date, i.latest_nav, u.policy
    ...
    codes = list(info)
    print(f"1-3m job within 1 month: {len(codes)}")
    ...
    out_rows = []
    for code in codes:
        _c, name, list_date, list_nav, policy = info[code]
        if code in dated_list:
            ...
            src = "price_api" if probe.get(code) == "ok" else ""
            fof_date = ""
            fof_nav = ""
    ...
    print("policy", dict(Counter(r["fof99_policy"] for r in out_rows)))
