/**
 * Fill missing product-page NAV dates from manager xlsx files (2026-09-14).
 * Only inserts dates absent from the current merged series.
 *
 * Usage: npx tsx scripts/ma/_fill_manager_nav_20260914.ts [--dry-run]
 */
import fs from "fs"
...
const FUNDS: Array<{
  ...
  {
    file: "11-1净值序列_20260831_100515_量宇红番茄十一号私募证券投资基金.xlsx",
    beian: "SJM483",
    name: "量宇红番茄十一号私募证券投资基金",
    fillManual: false,
  },
  {
    file: "诚奇中证500增强精选1期净值20260914.xlsx",
    beian: "SLS817",
    name: "诚奇中证500增强精选1期私募证券投资基金",
    fillManual: false,
  },
  {
    file: "朗坤鸿志量化1号净值20260914.xlsx",
    beian: "STR678",
    name: "朗坤鸿志量化1号私募证券投资基金",
    // Team series starts 2022-01-10, so private_fund_nav after that date is dropped.
    fillManual: true,
  },
  {
    file: "添禄投资添增2000二号C类净值20260914.xlsx",
    beian: "AAK83C",
    name: "添禄投资添增2000二号C类",
    fillManual: false,
  },
  {
    file: "华年进取2号净值20260914.xlsx",
    beian: "SANC63",
    name: "华年进取2号",
    fillManual: false,
  },
  {
    file: "六妙星滚雪粒1号A类净值20260914.xlsx",
    beian: "ASB96A",
    name: "六妙星滚雪粒1号A类",
    fillManual: false,
  },
...
async function main() {
  ...
  for (const fund of FUNDS) {
    ...
    const excelFirst = excel.rows[0]
    const excelLast = excel.rows.at(-1)
    const mergedLast = mergedBefore.at(-1)

    console.log(`\n==== ${fund.beian} ${fund.name}`)
    console.log(`file=${fund.file}`)
    ...
    console.log(
      `excel=${excel.validRowCount} first=${excelFirst?.date} last=${excelLast?.date} unit=${excelLast?.unitNav} cum=${excelLast?.cumulativeNav}`,
    )
    console.log(
      `merged=${mergedBefore.length} last=${mergedLast ? `${mergedLast.price_date.slice(0, 10)} unit=${mergedLast.nav}` : "none"} missing=${missing.length}`,
    )
    if (missing.length > 0) {
    ...
    if (missing.length === 0) {
      summary.push({ beian: fund.beian, missing: 0, upserted: 0, last: mergedLast?.price_date.slice(0, 10) })
      continue
    }
