#!/usr/bin/env python3
"""Export funds that currently match the 私募基金 list filter 净值日期 1个月以内.

Same columns as fof99_3_6m_have_data.csv. No paid calls.
"""
...
OUT = ROOT / "scripts" / "ma" / "fof99_current_1m_have_data.csv"
...
def main() -> int:
    ...
    cur.execute(
        ...
        WHERE latest_nav_date >= CURRENT_DATE - INTERVAL '1 month'
    ...
    codes = list(info)
    print(f"list filter 净值日期 1个月以内: {len(codes)}")
