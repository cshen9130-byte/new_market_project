...
def fetch_main(ts_code: str, start_date: str, end_date: str):
...
def main():
    ...
    start_date = "20230101"
    # default end_date: expected latest, allow override
    end_date = os.environ.get("SPOT_TRADE_DATE", "")
    if len(sys.argv) >= 3:
        start_date = sys.argv[1]
        end_date = sys.argv[2]
    if not end_date:
        # fallback to today ymd without weekend adjust; API caller sets end_date
    ...
    for key, ts_code in codes.items():
        data[key] = fetch_main(ts_code, start_date, end_date)
