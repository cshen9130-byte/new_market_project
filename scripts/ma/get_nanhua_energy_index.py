...
def main():
    ...
    # 鈹€鈹€ optional CLI args: start_date [end_date] in YYYYMMDD or YYYY-MM-DD 鈹€鈹€
    ...
    if args:
        ...
        # Default: last year Jan 1 鈫?today
    ...
    try:
        # 南华能化指数: NHECI.NH, field CLOSE, daily
        data = c.csd(
            "NHECI.NH",
            "CLOSE",
            start_date,
        ...
        print(json.dumps({"code": "NHECI.NH", "start": start_date, "end": end_date, "data": result, "meta": meta}, ensure_ascii=False))
    finally:
        try:
            logout = c.stop()
        except Exception:
