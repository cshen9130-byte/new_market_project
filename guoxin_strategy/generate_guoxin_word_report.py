...
CSV_FILES = {
    "account": BASE_DIR / "guosen_account_summary_full.csv",
    "trades": BASE_DIR / "guosen_transaction_records_full.csv",
    "positions": BASE_DIR / "guosen_position_summary_full.csv",
    "closed": BASE_DIR / "guosen_position_closed_full.csv",
}
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    account = pd.read_csv(CSV_FILES["account"])
    trades = pd.read_csv(CSV_FILES["trades"])
    positions = pd.read_csv(CSV_FILES["positions"])
    closed = pd.read_csv(CSV_FILES["closed"])
...
def fetch_market_history(trades: pd.DataFrame) -> pd.DataFrame:
    relevant = trades[["product", "instrument"]].drop_duplicates().copy()
    relevant["symbol"] = relevant["instrument"].str.upper()
    relevant["market"] = relevant["product"].map(PRODUCT_MARKET)

    start_date = (trades["trade_date"].min() - pd.Timedelta(days=45)).strftime("%Y%m%d")
    end_date = trades["trade_date"].max().strftime("%Y%m%d")
    frames: list[pd.DataFrame] = []

    for market in ["GFEX", "CZCE", "SHFE"]:
        needed = relevant.loc[relevant["market"] == market, "symbol"].unique().tolist()
        if not needed:
            continue
        market_df = ak.get_futures_daily(start_date=start_date, end_date=end_date, market=market)
        market_df["date"] = pd.to_datetime(market_df["date"].astype(str))
        market_df["symbol"] = market_df["symbol"].astype(str).str.upper()
        ...
        frames.append(market_df[market_df["symbol"].isin(needed)].copy())

    # DCE endpoint is less stable via get_futures_daily; use Sina daily history for the egg spread legs.
    dce_symbols = relevant.loc[relevant["market"] == "DCE", "symbol"].unique().tolist()
    for symbol in dce_symbols:
        daily_df = ak.futures_zh_daily_sina(symbol=symbol)
        daily_df["date"] = pd.to_datetime(daily_df["date"])
        daily_df["symbol"] = symbol
        daily_df["open_interest"] = pd.to_numeric(daily_df["hold"], errors="coerce")
        for col in ["open", "high", "low", "close", "volume", "settle"]:
            daily_df[col] = pd.to_numeric(daily_df[col], errors="coerce")
        daily_df["pre_settle"] = daily_df["settle"].shift(1)
        frames.append(daily_df[["date", "symbol", "open", "high", "low", "close", "volume", "open_interest", "settle", "pre_settle"]].copy())

    market_hist = pd.concat(frames, ignore_index=True).drop_duplicates(["date", "symbol"])
    return market_hist.sort_values(["symbol", "date"]).reset_index(drop=True)
...
def build_trade_clusters(trade_mkt: pd.DataFrame) -> dict:
    clusters: dict = {}
    open_trades = trade_mkt[trade_mkt["oc"] == "开"].sort_values(["trade_date", "product", "row_num"])
...
def build_close_clusters(closed: pd.DataFrame) -> dict:
    ...
    for (settlement_date, product), group in closed.groupby(["settlement_date", "product"]):
        agg = group.groupby(["instrument", "bs"], as_index=False).agg(lots=("lots", "sum"), realized=("realized_pl", "sum"))
...
def compose_analysis(account: pd.DataFrame, trades: pd.DataFrame, positions: pd.DataFrame, closed: pd.DataFrame, market_hist: pd.DataFrame, trade_mkt: pd.DataFrame) -> AnalysisResult:
...
def write_report(result: AnalysisResult) -> None:
    ...
    for (trade_date, product), cluster in sorted(result.trade_clusters.items(), key=lambda item: (item[0][0], item[0][1])):
        ...
        for _, row in cluster["summary"].iterrows():
            side = row["bs"]
            lines.append(
                f"{row['instrument']} {side} {int(row['lots'])} 手，均价 {row['avg_price']:.2f}，日内区间位置 {row['avg_range_pct']:.3f}，相对结算价执行优势 {row['avg_exec_vs_settle']:.2f}。"
            )
    ...
    add_table(
        ...
        [("日期", "settlement_date"), ("品种", "product"), ("多头", "long_lots"), ("空头", "short_lots"), ("净手数", "net_lots"), ("盯市盈亏", "mtm"), ("保证金", "margin")],
    )
...
def main() -> None:
    ...
    account, trades, positions, closed = load_data()
    ...
    if trade_mkt["settle"].isna().any():
        ...
        raise RuntimeError(f"以下合约未能匹配到行情数据: {missing}")
