...
export function valuationLedgerPairPrefix(parentCode: string, underlyingCode: string): string {
...
export function valuationLedgerGenerationKey(
  ...
  return `${valuationLedgerPairPrefix(parentCode, underlyingCode)}${date}`
}

/**
 * One 估值表 snapshot per parent fund per day.
 * The same workbook was sometimes stored twice with an off-by-one date; summing
 * those rows invented extra 申购 and a matching 赎回.
 */
const SQL_LATEST_VALUATION_RECORDS = `
  latest_valuation_records AS (
    SELECT DISTINCT ON (
      COALESCE(NULLIF(BTRIM(product_code), ''), NULLIF(BTRIM(fund_name), '')),
      valuation_date
    )
    ...
    FROM (
      SELECT DISTINCT ON (
        COALESCE(NULLIF(BTRIM(product_code), ''), NULLIF(BTRIM(fund_name), '')),
        COALESCE(
          NULLIF(BTRIM(regexp_replace(COALESCE(attachment_filename, ''), '^.*::', '')), ''),
          id::text
        )
      )
        id, product_code, fund_name, valuation_date
...
export function buildValuationLedgerRow(
  ...
  const absDelta = Math.abs(delta.deltaQty)
  const incrementNav =
    delta.deltaCost != null && Math.abs(delta.deltaQty) >= MIN_ABS_SHARES
      ? Math.abs(delta.deltaCost) / Math.abs(delta.deltaQty)
      : null
  const nav = confirm?.unitNav ?? delta.prevNav ?? incrementNav ?? delta.nav
  ...
  if (delta.firstAppearance) remarks.push("估值表首现持仓")
  if (!confirm) {
    const navNote = delta.prevNav != null ? "净值用申请日行情" : "净值用申购成本/份额"
    remarks.push(`由估值表数量/成本变动估算，申请日=上一估值日，${navNote}`)
  } else {
    remarks.push(`已匹配交易确认单#${confirm.id}`)
  }
...
function mergeHoldingPointsByDate(points: HoldingPoint[]): Map<string, HoldingPoint> {
  const byDate = new Map<string, HoldingPoint>()
...
export function collectValuationLedgerDeltas(input: {
  parentCode: string
  parentName: string
  parentKey: string
  underlyingCode: string
  points: HoldingPoint[]
  /** Parent 估值日. Dates with no holding are qty=0 so a full exit still emits 赎回. */
  parentDates?: string[]
}): ValuationLedgerDelta[] {
  const byDate = mergeHoldingPointsByDate(input.points)
  const fallbackName = input.points.find((p) => p.undName)?.undName || input.underlyingCode
  const dates = [...new Set([...byDate.keys(), ...(input.parentDates ?? [])])].sort()
    ...
    if (prev == null) {
      if (isMeaningfulShareDelta(point.qty, nav)) {
        deltas.push({
          ...
          underlyingCode: input.underlyingCode,
...
type HeldIndex = {
  byBeian: Map<string, HeldProduct>
  byFamily: Map<string, HeldProduct[]>
  byName: Map<string, HeldProduct[]>
}

function buildHeldIndex(held: HeldProduct[]): HeldIndex {
  const byBeian = new Map<string, HeldProduct>()
  const byFamily = new Map<string, HeldProduct[]>()
  const byName = new Map<string, HeldProduct[]>()
  for (const row of held) {
    byBeian.set(row.beian_hao, row)
    const familyList = byFamily.get(row.family) ?? []
    familyList.push(row)
    byFamily.set(row.family, familyList)
    const name = fundNameBase(row.product_name)
    if (!name) continue
    const nameList = byName.get(name) ?? []
    nameList.push(row)
    byName.set(name, nameList)
  }
  return { byBeian, byFamily, byName }
}

function pickHeld(
  symbol: string,
  undName: string | null | undefined,
  index: HeldIndex,
): HeldProduct | null {
  const upper = symbol.toUpperCase()
  const exact = index.byBeian.get(upper)
  ...
  if (family) {
    const byFamily = best(index.byFamily.get(family) ?? [])
    if (byFamily) return byFamily
  }
  if (name) {
    const byName = best(index.byName.get(name) ?? [])
    if (byName) return byName
  }
  if (!upper) return null
  return {
    beian_hao: upper,
    product_name: (undName && stripValuationSubjectPathPrefix(undName)) || upper,
    family: family || upper,
  }
}
...
export async function generateFofUnderlyingLedgerFromValuation(): Promise<GenerateValuationLedgerResult> {
  ...
  // Include cleared underlyings (market_value null/0). Filtering to current 市值>0
  // dropped historical 申购 and the 赎回 that took them off the book.
  ...
  const heldIndex = buildHeldIndex(held)
  ...
  const series = await queryUnbounded<SeriesRow>(
    `WITH ${SQL_LATEST_VALUATION_RECORDS}
     SELECT
     ...
     INNER JOIN latest_valuation_records r ON r.id = h.valuation_record_id
     WHERE ${HOLDING_FILTER_SQL}
       AND EXISTS (
         SELECT 1
           FROM ops_fof_overview_list_cache c
          WHERE NULLIF(BTRIM(c.beian_hao), '') IS NOT NULL
            AND (
              ${sqlBeianFamilyKey("h.symbol")} = ${sqlBeianFamilyKey("c.beian_hao")}
              OR UPPER(BTRIM(h.symbol)) = UPPER(BTRIM(c.beian_hao))
              OR (
  ...
  for (const row of series) {
    const und = pickHeld(row.und_code, row.und_name, heldIndex)
    if (!und) continue
    ...
    const key = `${parentKey}::${und.family}`
    let bucket = grouped.get(key)
    if (!bucket) {
      bucket = { parentCode: parent.code, parentName: parent.name, parentKey, und, points: [] }
      grouped.set(key, bucket)
    } else if (heldMatchRank(und, fundNameBase(row.und_name)) > heldMatchRank(bucket.und, fundNameBase(row.und_name))) {
      bucket.und = und
      bucket.parentCode = parent.code
      bucket.parentName = parent.name
    }
  ...
  const parentCodes = [...new Set(
    [...grouped.values()].map((b) => b.parentCode).filter((code) => Boolean(code)),
  )]
  const parentDatesByKey = new Map<string, string[]>()
  if (parentCodes.length > 0) {
    const parentDateRows = await queryUnbounded<{
      parent_code: string | null
      parent_name: string | null
      valuation_date: string
    }>(
      `WITH ${SQL_LATEST_VALUATION_RECORDS}
       SELECT
         NULLIF(BTRIM(product_code), '') AS parent_code,
         NULLIF(BTRIM(fund_name), '') AS parent_name,
         valuation_date::text AS valuation_date
       FROM latest_valuation_records
       WHERE UPPER(BTRIM(COALESCE(product_code, ''))) = ANY($1::text[])`,
      [parentCodes.map((code) => code.toUpperCase())],
    )
    for (const row of parentDateRows) {
      const rawParent = String(row.parent_code || "").trim().toUpperCase()
      const rawParentName = String(row.parent_name || "").trim()
      if (!rawParent && !rawParentName) continue
      const parent = mapParent(rawParent, rawParentName, tracking)
      ...
      const date = String(row.valuation_date || "").slice(0, 10)
      if (!date) continue
      const list = parentDatesByKey.get(parentKey) ?? []
      list.push(date)
      parentDatesByKey.set(parentKey, list)
    }
  }
  ...
  for (const bucket of grouped.values()) {
    processedPrefixes.add(valuationLedgerPairPrefix(bucket.parentKey, bucket.und.beian_hao))
    deltas.push(
      ...collectValuationLedgerDeltas({
        ...
        parentDates: parentDatesByKey.get(bucket.parentKey),
      }),
    )
  }
  ...
  const toWriteById = new Map<string, OpsLedgerRow>()
  ...
  for (const delta of deltas) {
    ...
    if (toWriteById.has(id)) continue
    if (skips.has(id)) {
      skippedDeleted += 1
      continue
    }
    ...
    toWriteById.set(id, buildValuationLedgerRow(delta, confirm))
    if (prev) updated += 1
    else inserted += 1
  }
  ...
  const generatedIds = new Set(toWrite.map((row) => row.id))
  const staleIds = existing
    .filter((row) => {
      if (!isAutoGeneratedLedgerSource(row.source) || isProtectedLedgerRow(row)) return false
      const key = row.generation_key || row.id
      if (![...processedPrefixes].some((prefix) => key.startsWith(prefix))) return false
      return !generatedIds.has(row.id)
    })
    .map((row) => row.id)
  const removedStale = await deleteUnprotectedAutoLedgerRecords(staleIds)
