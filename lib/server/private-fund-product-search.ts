 ...
 */
export async function searchPrivateFundProductsForAutocomplete(
  q: string,
  limit = 20,
): Promise<PrivateFundPickerResult[]> {
  ...
  if (registerCode) {
    return query<PrivateFundPickerResult>(
      `SELECT beian_hao, product_name, NULL::text AS short_name, strategy_l1 AS strategy_one
       FROM private_fund_info
       WHERE beian_hao = $1 OR beian_hao ILIKE $2
       ORDER BY product_name ASC
       LIMIT $3`,
      [registerCode, `${registerCode}%`, limit],
    ).catch((err) => {
      console.error("[private-fund-product-search] autocomplete by register", err)
      return []
    })
  }

  // Match fund keyword autocomplete exactly (prefix on private_fund_info).
  const prefix = `${trimmed}%`
  return query<PrivateFundPickerResult>(
    `SELECT beian_hao, product_name, NULL::text AS short_name, strategy_l1 AS strategy_one
     FROM private_fund_info
 ...
 * Prefer searchPrivateFundProductsForAutocomplete for typing UIs.
 */
