...
- fee_pay: 业绩报酬说明，一两句：计提比例、份额差异、计提时点。不要整章费用与税收。R>0% 只是超额计提起点，不是业绩基准；合同写明「计提基准为年化 X%」才填业绩基准。
- fee_pay_formula: 只保留公式本身，一两行，如「R>6%时提取40%。R=(A-B)/C×365/N；E=K×T1×(R-B)×T/365×40%。」禁止费用种类清单、管理费/托管费公式、页码。A/B/C 比例不同时按类别列出。
