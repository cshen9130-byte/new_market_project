...
let inflight: Promise<CffexProductSnapshot[]> | null = null
let cached: { at: number; data: CffexProductSnapshot[] } | null = null
const TTL_MS = 1500
...
export async function getCffexIndexRealtime() {
  ...
  inflight = (async () => {
    const hqText = await sinaGet(
      `https://hq.sinajs.cn/list=${INDEX_FUTURES.map((item) => `nf_${SINA_SYMBOL[item.product]}`).join(",")}`,
      "https://finance.sina.com.cn",
    )
    const quotes = parseHq(hqText)
    return Promise.all(INDEX_FUTURES.map((item) => fetchProduct(item.product, item.name, quotes)))
  })().finally(() => {
    inflight = null
  })
