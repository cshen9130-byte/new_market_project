...
export function HelpJiaama() {
  return (
    <ChartHelp title="只做加码同向">
      <p>把页面上的「加码」译成 2,000 万模拟期货账户：量化与主观存量同向且都够大，才与共识同向开仓。控拥挤、观望、暂缓加码、减码准备、补风格一律空仓；信号不再是加码时，下一交易日平掉。</p>
      <Formula>仓位 ≈ 当日权益 × 2.2 倍名义 / 最多 8 个品种等权</Formula>
      <Formula>单品种 1σ 亏损 ≤ 权益 1.2% · 保证金占用 ≤ 权益 50%</Formula>
      <Formula>单边手续费 1.2bp + 滑点 1.5bp，每手手续费不低于 3 元，开平都收</Formula>
      <p>信号日收盘后生成目标仓，下一交易日生效。有夜盘的品种最早当晚 21:00 下单；苹果/鸡蛋等无夜盘则次日 09:00。只在重建出非中性信号的那天调仓；全是中性的日子继续拿着旧仓。</p>
      <p>持仓盯主力合约。主力换月当天按旧约平、新约开，双边收手续费和滑点；当日盈亏用该合约自己的收盘价，不用连续合约跳空。</p>
      <p>净值随 MOM 持仓与行情更新（按日缓存）。这是研究账户，不是券商结算单：没有涨跌停、排队和真实换月滑点加宽。</p>
    </ChartHelp>
  )
}
