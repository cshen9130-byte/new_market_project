...
function getPageContext(path: string, search = ""): string {
  ...
  if (path.includes("/tools/valuation-table-analysis"))
    return "当前页面：【估值表分析】小工具。功能：搜索并选择产品后，上传估值表（xls/xlsx）解析入库，与「私募基金 → 运维 → 估值表管理」一致，支持打开估值表页核对。"
  ...
  if (path.includes("/macro-market"))
    return "当前页面：【宏观市场分析】。国内三个模块：①PCA 聚类模型 — 基于宏观因子（CPI、PMI、M1、信贷等）的主成分分析双标图；②经济体制相似性模型 — 当前宏观与历史时期的欧氏距离及对应资产表现；③货币+信用 周期模型 — 货币与信用四象限轮动。全球 → 美国：FOMC / CPI 每日跟踪 — 按日更新下次决议定价、委员会当日已有的 CPI 同比/环比、一致预期、FedWatch 加息概率、非农、失业率（U-3/U-6、Sahm 规则、劳动参与率）与核心分项。"
  ...
  if (path.includes("/realtime-quotes"))
    return "当前页面：【实时行情】。展示股指期货 IH/IF/IC/IM 的 1 分钟 K 线。浏览器经同源 API 读取服务端 ctp_market（SimNow CTP MdApi 聚合成 1 分钟 OHLCV）。K 线从行情进程启动后开始累计。"
  if (path.includes("/private-funds/") && path.includes("/settlement"))
    return "当前页面：【结算单分析】。功能：为当前产品拖入结算单（国信盯市或监控中心/期货公司日报）解析持仓敞口、板块与策略；也可关联中国期货市场监控中心投资者查询服务系统自动获取结算日报后再分析。"
  ...
  if (path.includes("/event-calendar"))
    return "当前页面：【事件日历】。展示华尔街见闻财经日历中可能影响股债汇商的宏观数据、央行决议与休市安排，含今值、预期与前值，支持按日期筛选与查看详情。"
