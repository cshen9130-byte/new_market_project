...
for s, e in daterange_chunks(start_date, end_date, CHUNK_DAYS):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            gov = df[df["曲线名称"] == CURVE_NAME][[DATE_COL, COL_1Y, COL_10Y]]
            if not gov.empty:
                frames.append(gov)
            print(f"Fetched {s_str}~{e_str}: {len(gov)} rows")
            break
...
raw[COL_1Y]  = pd.to_numeric(raw[COL_1Y],  errors="coerce")
raw[COL_10Y] = pd.to_numeric(raw[COL_10Y], errors="coerce")
raw = raw.dropna(subset=[COL_1Y, COL_10Y]).sort_values(DATE_COL).set_index(DATE_COL)

raw["10Y_1Y_spread"] = raw[COL_10Y] - raw[COL_1Y]

# Resample to monthly mean
monthly = raw[["10Y_1Y_spread", COL_1Y, COL_10Y]].resample("ME").mean().reset_index()
monthly.columns = ["date", "10Y_1Y_spread", "yield_1y", "yield_10y"]
monthly["date"] = monthly["date"].dt.strftime("%Y/%m/%d")

output_path = Path(__file__).parent / "data" / "china_10y1y_spread_monthly.csv"
