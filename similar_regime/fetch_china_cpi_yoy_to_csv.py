...
def _extract_rows(edb_result):
    # EmQuant edb returns a mapping of code -> value series and matching date series.
...
def main():
    ...
    try:
        data = c.edb("EMM00072301", "IsLatest=0,StartDate=2000-01-01,EndDate=2026-03-14")
        if data.ErrorCode != 0:
            print(f"Fetch failed: {data.ErrorMsg}")
            return

        rows = _extract_rows(data)
        output_path = Path(__file__).parent / "data" / "china_cpi_yoy_monthly.csv"
