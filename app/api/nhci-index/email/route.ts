...
import {
  publicEmailConfig,
  readEmailConfig,
  runDueNhciIndexEmails,
  sendNhciIndexEmail,
  testSenderConnection,
  writeEmailConfig,
  type ExtraEmailAttachment,
} from "@/lib/server/nhci-index-email"
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (body.sender) {
      ...
      sender = {
        name: String(body.sender.name || "").trim() || "NHCI指数跟踪",
    ...
    const due = await runDueNhciIndexEmails()
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const result = await sendNhciIndexEmail({ extraAttachments: extras })
    ...
    const result = await sendNhciIndexEmail()
    return NextResponse.json({ ok: true, ...result, config: publicEmailConfig(readEmailConfig()) })
  } catch (e) {
