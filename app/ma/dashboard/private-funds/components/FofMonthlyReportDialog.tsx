...
import {
  deleteFofMonthlyReportPreset,
  loadFofMonthlyReportPresets,
  upsertFofMonthlyReportPreset,
  type FofMonthlyReportPreset,
} from "@/lib/client/fof-monthly-report-presets"
...
function latestMonthRange(latestNavDate: string): { begin: string; end: string } {
  return {
    begin: monthBegin(latestNavDate),
    end: latestNavDate,
  }
}
...
export function FofMonthlyReportDialog({
  open,
  onClose,
  onBack,
  embedded = false,
  ...
  const [navFrequency, setNavFrequency] = useState<NavFrequency>("monthly")
  const [productSource, setProductSource] = useState<ProductSource>("managed")
  const [presets, setPresets] = useState<FofMonthlyReportPreset[]>([])
  ...
  function applyPreset(preset: FofMonthlyReportPreset) {
    ...
    setMonthBeginDate(preset.month_begin)
    setMonthEndDate(preset.month_end)
  ...
  function handleDeletePreset() {
    ...
    const next = deleteFofMonthlyReportPreset(name)
    setPresets(next)
    setSelectedPresetName("")
  ...
  useEffect(() => {
    ...
    fetch(`/ma/api/reports/fof-monthly/nav-range?${params}`)
      .then((r) => r.json())
      .then((json) => {
  ...
  function handleFillLatestMonth() {
    if (!navRange.end) {
      setError("请先选择有净值数据的产品")
      return
    }
    const range = latestMonthRange(navRange.end)
  ...
  async function handleGenerate() {
    ...
    if (navRange.start && monthBeginDate < navRange.start) {
      setError(`开始日期不能早于 ${navRange.start}`)
      return
    }
    if (navRange.end && monthEndDate > navRange.end) {
          ...
          {result && (
            <div className="mt-6 rounded-xl border bg-zinc-50/80 p-4 dark:bg-zinc-900/40">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold">{result.reportTitle} 月报</h3>
