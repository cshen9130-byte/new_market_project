...
function buildScaleTrendOption(scaleTrend: PeIndustryScaleTrendPoint[]): Record<string, unknown> {
  const months = scaleTrend.map((p) => p.month)
  return {
    ...
    series: PE_INDUSTRY_SCALE_TREND_BUCKETS.map((bucket, index) => ({
      ...
      data: scaleTrend.map((p) => p.counts[bucket as PeIndustryScaleTrendBucket]),
    })),
  }
}
...
function exportChangesCsv(rows: PeIndustryManagerScaleChange[]) {
...
export function PeIndustryManagerScaleSection({
  ...
  scaleChanges: { updatedAt: string; rows: PeIndustryManagerScaleChange[] }
}) {
  const [page, setPage] = useState(1)
  const trendOption = useMemo(() => buildScaleTrendOption(scaleTrend), [scaleTrend])

  const rows = scaleChanges.rows
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-lg border border-zinc-100 bg-white px-4 py-4">
        ...
        {scaleTrend.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-[340px] text-sm text-zinc-400">
            暂无规模走势历史数据（需 nightly ETL 积累 amac_manager_metrics_history）
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg border border-zinc-100 bg-white overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-zinc-100">
          ...
          <div className="flex items-center gap-3 shrink-0">
            {scaleChanges.updatedAt && (
              <div className="text-xs text-zinc-500">
                更新日期：{scaleChanges.updatedAt}
              </div>
            )}
            <button
              type="button"
