...
const CHECKS_HELP: ChartCalcHelpBlock[] = [
  ...
  {
    title: "第41条 产品类别（国泰君安托管202607）",
    paragraphs: ["分母一律是托管口径「已投资产」= 估值表资产合计 − 现金管理工具。合约价值不计入分母。"],
    bullets: [
      "权益类：（1102股票 + 1105股票类基金 + 穿透私募股票 − 2101融券）/ 已投资产 ≥ 80%",
      "固定收益类：债权类（信用债、逆回购、债券基金、可转债）/ 已投资产 ≥ 80%",
      "期货和衍生品类：场内合约价值 / 已投资产 ≥ 80%，且账户权益 / 同一已投资产 ≥ 20%",
      "混合类：以上三类都未达标",
    ],
  },
  {
...
const MIX_HELP: ChartCalcHelpBlock[] = [
  {
    title: "已投资产（分母）",
    paragraphs: [
      "按国泰君安托管202607：第41条分母是产品估值表资产合计减去现金管理工具，不是净资产，也不把期货合约价值加进分母。",
    ],
    formula: "已投资产 = 资产合计 − 托管户活期(100201) − 券商普通资金账户(103106) − 国债/央票/政金债/地方债 − 货币基金",
  },
  {
    title: "市值 vs 合约价值 vs 保证金",
    bullets: [
      "权益分子只计 1102 股票、1105 股票类基金，私募 1109 往下穿透股票；再减去 2101 融券负债。可转债、信用账户包装、券商保证金不计权益",
      "现金管理工具不含国债逆回购、信用账户资金、期货保证金/备付金——这些留在已投资产里",
      "期货合约价值只计场内初始合约（3102/3201 所列科目）和期权数量×乘数×收盘价；冲销/估值增值不计。场外 1114 托管未自动化，暂不计入分子",
      "账户权益 = 102113+103113+102131+103131+103133，与合约价值除以同一已投资产",
      "图上期货占比是合约价值/已投资产，不是已投资产的一块切片，因此各类占比不必加总为 100%",
    ],
  },
  {
    title: "各类怎么归",
    bullets: [
      "权益类：1102 股票、1105 股票/混合/指数/ETF，以及穿透后的底层股票",
      "债权类：信用债、逆回购、债券基金、可转债；国债/央票/政金债/地方债改记现金管理工具",
      "期货和衍生品：场内初始合约价值（多头+|空头|），不含保证金、冲销、场外 1114",
      "基金（未穿透）：底层私募没有估值表，整段份额按基金计，仍在已投资产里",
      "现金管理工具：托管户活期、券商普通资金账户、国债、央票、政金债、地方债、货基；从资产合计中扣除",
      "期货/期权保证金与备付金：计入已投资产，同时作为账户权益分子",
    ],
  },
  {
    title: "图上的百分比",
    formula: "占比 = 该类金额 / 已投资产",
    paragraphs: ["现金管理工具单独列出扣除额。期货棒是合约价值相对已投资产，不要求与权益/债权加总为 100%。"],
  },
]
...
const SUBFUND_HELP: ChartCalcHelpBlock[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "底层期货名义：该底层估值表上的双边名义（数量×行情×合约乘数），未按母基金份额缩放",
      "权益/债权/期货/基金/现金/其他：与上方同一套国泰君安托管口径；期货是分成后的母基金切片。占比=该行该类金额/该行已投资产",
      "已投资产：该行权益+债权+期货账户权益+未穿透基金+其他（不含现金管理工具，也不含合约价值）",
      "占母基金：该行已投资产 / 母基金已投资产（母基金已投资产=资产合计−现金管理工具）",
    ],
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-zinc-500">
              依据《私募证券投资基金运作指引》（2024-08-01），第41条按国泰君安托管202607口径。FOF 持仓按底层最新估值表穿透后核验。
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-zinc-100 p-4">
            ...
            <p className="mb-2 text-[11px] leading-4 text-zinc-400">
              已投资产 = 资产合计 − 现金管理工具（国泰君安托管202607）。期货棒是场内合约价值/已投资产，不计入分母。
            </p>
            <div className="space-y-2">
              {mixRows.map((row) => {
                const anomaly =
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-zinc-500">
              ...
              {marginPosted > 0 && (
                <div>保证金/备付金 {fmtMoney(marginPosted)}（期货/期权保证金计入已投资产与账户权益）</div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-zinc-100 overflow-hidden">
          <div className="border-b border-zinc-100 px-4 py-3">
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400">
              「底层期货名义」是该底层自己的双边合约价值。「期货和衍生品」已乘分成，各行相加应等于合计。可上传该底层更新的估值表后自动重算。
            </p>
            <input
              ref={fileInputRef}
              type="file"
