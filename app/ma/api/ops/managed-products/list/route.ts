...
export async function GET(req: Request) {
  try {
    ...
    if (useCache) {
      ...
      const baseFrom = buildManagedProductsFrom(PRODUCT_EXPR).replace(
      ...
      const rows = await query<{
        ...
        `SELECT * FROM (
           SELECT
             m.id::text AS id,
             m.sequence_no,
             COALESCE(cache.beian_hao, ${BEIAN_EXPR}) AS beian_hao,
             m.product_name,
             COALESCE(cache.short_name, ${SHORT_EXPR}) AS short_name,
             ${strategyExpr} AS strategy_l1,
             cache.unit_nav::text AS latest_unit_nav,
             cache.nav_date AS latest_nav_date,
    ...
    const currentNavExpr = navExpr
    const currentDateExpr = dateExpr
    const currentPctExpr = pctExpr

    const histJoins = [7, 30, 90, 180, 365]
      .map((days, i) => {
        const aliases = ["h1w", "h1m", "h3m", "h6m", "h1y"]
        return managedNavAtOffsetJoin(aliases[i], BEIAN_EXPR, PRODUCT_EXPR, SHORT_EXPR, days, cutoffExpr)
      })
      .join("\n")
