...
async function _GET(req: Request) {
  ...
  const histDays = Math.max(20, Math.min(756, parseInt(searchParams.get("histDays") ?? "252", 10)))
  // alpha = confidence level, e.g. 0.95 → tail = worst 5% of simulated P&L days
  const alpha = Math.max(0.5, Math.min(0.99, parseFloat(searchParams.get("alpha") ?? "0.95")))
  ...
  try {
    ...
    //    We use |componentCVaR_i| as the absolute contribution so both long/short
    //    positions show their tail-risk exposure regardless of direction.
    type RowData = {
      date: string
    ...
    for (const date of tradingDates) {
      ...
      if (mktIdx < histDays) continue

      const histDatesArr = allMktDates.slice(mktIdx - histDays, mktIdx)
      ...
      // Return series per product for the historical window
      const retSeries = prodMvs.map(({ prod }) => {
        const code = AKSHARE_CODE[prod]
        const m = pctMap.get(code ?? "")
        return histDatesArr.map(d => m?.get(d) ?? 0)
      })

      // Portfolio P&L for each historical date
      const portPnl = histDatesArr.map((_, t) => {
        let pnl = 0
        for (let i = 0; i < N; i++) pnl += prodMvs[i].mv * retSeries[i][t]
        return pnl
      })

      // Identify tail: worst floor(histDays * (1-alpha)) observations by portfolio P&L
      const tailCount = Math.max(1, Math.floor(histDays * (1 - alpha)))
      const sortedIdxByPnl = portPnl.map((v, i) => ({ v, i })).sort((a, b) => a.v - b.v)
      const tailIndices = new Set(sortedIdxByPnl.slice(0, tailCount).map(x => x.i))

      // Component CVaR_i = -mean(mv_i * r_{i,t}) for t in tail
      // Use absolute value: shows how much each product contributes to tail losses
      ...
      for (let i = 0; i < N; i++) {
        let tailSum = 0
        for (const idx of tailIndices) tailSum += prodMvs[i].mv * retSeries[i][idx]
        const componentCVaR = tailCount > 0 ? -(tailSum / tailCount) : 0
        const absCVaR = Math.abs(componentCVaR)
        if (absCVaR < 1) continue
        ...
        catMcr[cat]       = (catMcr[cat]       ?? 0) + absCVaR
        sectorMcr[sector] = (sectorMcr[sector] ?? 0) + absCVaR
        subMcr[sub]       = (subMcr[sub]       ?? 0) + absCVaR
        totalMcr += absCVaR
      }
...
export const GET = withMomCache("cvar-sector-timeseries", _GET)
