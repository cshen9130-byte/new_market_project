...
export async function GET(req: Request) {
  try {
    ...
    const countRows = await query<{ n: string }>(
      `SELECT COUNT(*)::text AS n FROM investment_tracking_products ${where}`,
      params,
    )
    ...
    const rows = await query<{
      ...
      `SELECT
         ...
         ret_6m,
         ret_1y
       FROM investment_tracking_products
       ${where}
    ...
    const data: TrackingProductRow[] = rows.map((r) => ({
      id: r.id,
      seq_no: r.seq_no,
    ...
    console.error("[tracking-products/list]", err)
    return NextResponse.json({ error: "Failed to load tracking products" }, { status: 500 })
  }
}
