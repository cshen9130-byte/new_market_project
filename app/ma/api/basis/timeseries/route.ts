...
export async function GET(req: Request) {
  ...
  // Cache-first: return cached content when preferCache is set; otherwise require it to be fresh
  try {
    ...
    if (buf) {
      ...
      if (!forceRecompute) {
        if (end && end >= expectedYmd && obj?.data && calc === "settle") {
          return json(obj, 200)
        }
      }
    }
  ...
  for (const code of codes) {
    ...
    const series: Array<{ date: string; annualized_basis_pct: number | null; spot_close?: number; futures_settle?: number; days_to_maturity?: number }> = []
    for (const f of futSeries) {
      ...
      if (typeof spotClose === "number" && typeof futSettle === "number") {
        const expiry = nearestThirdFridayAfter(iso)
        daysToMat = daysBetween(iso, expiry)
        const basis = (futSettle - spotClose) / spotClose
        annualized = basis * (365 / (daysToMat || 1)) * 100
      }
      series.push({ date: iso, annualized_basis_pct: annualized, spot_close: spotClose, futures_settle: futSettle, days_to_maturity: daysToMat ?? undefined })
    }
    out.data[code] = series
    // Log inputs for end date (latest) to compare with far-basis, and attach if debug
    try {
      const endIso = ymdToIso(endYmd)
      const match = series.find((d) => d.date === endIso) || series.at(-1)
      const dbg = {
        date: match?.date || null,
        futures_settle: match?.futures_settle ?? null,
        spot_close: match?.spot_close ?? null,
        days_to_maturity: match?.days_to_maturity ?? null,
      }
      debugInputs[code] = dbg
      console.log("[basis-ts] inputs", code, dbg)
    } catch {}
  }

  try {
    const toWrite = { ...out, calc: "settle" }
    await fs.promises.writeFile(cachePath, JSON.stringify(toWrite, null, 2), "utf-8")
  } catch (e) {
    console.warn("[basis-timeseries] failed to write cache:", (e as any)?.message)
  }

  const payloadBase = { ...out, calc: "settle" }
