...
export async function GET(req: Request) {
  ...
  const futScript = path.join(process.cwd(), "scripts/ma/get_cffex_index_futures_continuous_range.py")
  const spotScript = path.join(process.cwd(), "scripts/ma/get_spot_indices_timeseries.py")
  const cacheDir = path.join(process.cwd(), "data")
  const cachePath = path.join(cacheDir, "basis_cont_diff_timeseries_cache.json")
  ...
  for (const code of codes) {
    ...
    for (const leg of legs) {
      const futSeries: Array<{ trade_date: string; close: number | null; settle: number | null }> = futRes?.data?.[code]?.[leg] || []
    ...
    out.data[code] = legOut
  }

  try {
    await fs.promises.writeFile(cachePath, JSON.stringify(out, null, 2), "utf-8")
  } catch (e) {
    console.warn("[basis-cont-diff-timeseries] failed to write cache:", (e as any)?.message)
  }
