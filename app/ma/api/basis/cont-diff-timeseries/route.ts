...
export async function GET() {
  try {
    const rows = await query<{
      ...
      `SELECT symbol, leg, trade_date, basis_diff, spot_close, futures_settle
       FROM derived_basis_cont_daily
       ORDER BY symbol, leg, trade_date ASC`
    )
    if (!rows.length) return NextResponse.json({ error: "No data" }, { status: 404 })
    const data: Record<string, Record<string, unknown[]>> = {}
    for (const r of rows) {
      if (!data[r.symbol]) data[r.symbol] = {}
      if (!data[r.symbol][r.leg]) data[r.symbol][r.leg] = []
      data[r.symbol][r.leg].push({
