...
function thirdFriday(year: number, month: number): Date {
  // Find the third Friday of given month
  const first = new Date(year, month - 1, 1)
  const firstDay = first.getDay() // 0=Sun..5=Fri..6=Sat
...
function hasCompleteData(entry: any): boolean {
  ...
  // strictly require settle-based cache entries
...
export async function GET(req: Request) {
  ...
  // Cache-first: if we already have latest entry for expected trade date, return it
  ...
  if (spotRes?.error || !spotRes?.data || Object.keys(spotRes.data).length === 0) {
    // Fallback to Tushare index_daily for spot closes
  ...
  // Debug logs for spot and futures
  try {
    console.log("[basis-far] tradeDate", tradeDateIso)
    console.log("[basis-far] spotRes", spotRes)
    const dbg: any = {}
    for (const c of ["IH", "IF", "IC", "IM"]) {
      const f = futRes?.data?.[c]
      dbg[c] = { ts_code: f?.ts_code, close: f?.close }
    }
    console.log("[basis-far] fut main", dbg)
  } catch {}
  ...
  for (const code of codes) {
    ...
    // Use main contract as "far" per requirement
    const farTs: string | undefined = f?.ts_code
    const farSettle: number | undefined = typeof f?.settle === "number" ? f.settle : undefined
    ...
    if (farTs && typeof farSettle === "number" && typeof spotClose === "number") {
      const exp = parseExpiryFromTsCode(farTs)
      let expiry: Date | null = null
      if (exp) {
        expiry = thirdFriday(exp.year, exp.month)
      } else {
        // Fallback for continuous main contracts like IHL1.CFX: use nearest upcoming third Friday
      ...
      if (expiry) {
        ...
        const basis = (farSettle - spotClose) / spotClose
        annualizedPct = basis * (365 / (daysToMat || 1)) * 100
      }
    }
    const dbg = {
      trade_date: tradeDateIso,
      far_settle: farSettle ?? null,
      spot_close: spotClose ?? null,
      days_to_maturity: daysToMat ?? null,
      expiry: expiryStr,
    }
    debugInputs[code] = dbg
    try { console.log("[basis-far] inputs", code, dbg) } catch {}
    out[code] = {
      trade_date: tradeDateYmd,
      far_ts_code: farTs || null,
      far_close: typeof f?.close === "number" ? f.close : null,
      far_settle: farSettle ?? null,
      spot_close: spotClose ?? null,
  ...
  try {
    ...
    console.warn("[basis-far] failed to write cache:", (e as any)?.message)
  }
