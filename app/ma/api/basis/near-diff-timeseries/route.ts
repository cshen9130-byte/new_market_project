...
export async function GET(req: Request) {
  ...
  const cachePath = path.join(cacheDir, "basis_near_diff_timeseries_cache.json")
  ...
  for (const code of codes) {
    ...
    try {
      ...
      console.log("[basis-near-diff-ts] inputs", code, dbg)
    } catch {}
  }
  ...
  try {
    ...
    console.warn("[basis-near-diff-timeseries] failed to write cache:", (e as any)?.message)
  }
